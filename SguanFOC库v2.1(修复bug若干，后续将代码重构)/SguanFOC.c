 ...
 * @LastEditTime: 2026-01-26 21:50:37
 * @FilePath: \demo_SguanFOCCode\SguanFOC\SguanFOC.c
...
// 读取零电流ADC数据
static void Sguan_ReadADC_Offset(Motor_System_STRUCT *sguan,uint32_t Motor_CH) {
    sguan->Current.Offset_a = SguanUser_ReadADC_Raw(Motor_CH,0);
    sguan->Current.Offset_c = SguanUser_ReadADC_Raw(Motor_CH,2);
}
...
static void Sguan_Current_Calculate(Motor_System_STRUCT *sguan,uint8_t Motor_CH) {
    ...
    if (sguan->Current.Dir_b) {
        sguan->Current.Real_Iv = sguan->Current.Real_Iu+sguan->Current.Real_Iw;
    }
    else {
        sguan->Current.Real_Iv = -(sguan->Current.Real_Iu+sguan->Current.Real_Iw); 
    }
    ...
    sguan->Current.Real_Id = low_pass_filter(sguan->Current.Raw_Id,sguan->Current.Real_Id,sguan->Current.Id_Filter);
    sguan->Current.Real_Iq = low_pass_filter(sguan->Current.Raw_Iq,sguan->Current.Real_Iq,sguan->Current.Iq_Filter);
}
